 ...
 * Date   : 10.31.2007
 *
 * This tutorial will show you how to use Lzo Archive Plugin in Edge
 *
 */
#include "Edge.h"
...
public:
	...
	{
#ifdef EDGE_DEBUG
		mPluginFile = "config/04.plugin_d.config";
#else
		mPluginFile = "config/04.plugin.config";
#endif
		mCaption = "Edge Example 04 Read Data From A Lzo Archive";
	}
...
bool MyApp::init()
{
	...
	am->createArchive( "./media/res.lzo", "Lzo" );

	/// create images, note the param, it contains the archive name and the file name.
	mImgLogo = ifac->createImage( "./media/res.lzo:edge_logo.jpg" );

	/// create sound
	mSndBack = sfac->createSound( "./media/res.lzo:getout.ogg" );
	mSndClick = sfac->createSound( "./media/res.lzo:logo.wav" );
	...
	/// create fonts
	TTFontParam param;
	mFont = EdgeEngine::getSingleton().getFontFactory()->createFont( "font", "./media/res.lzo:font.fnt" );
