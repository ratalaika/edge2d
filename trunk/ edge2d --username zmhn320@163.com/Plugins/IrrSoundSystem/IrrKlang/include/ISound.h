...
namespace audio
{
	...
	public:
		...
		virtual bool setPlaybackSpeed(f32 speed=1.0f) = 0;
		...
		If the ISound pointer gets dropped (IVirtualUnknown::drop()), the ISoundEffects
		...
		//! Sets the sound stop event receiver, an interface which gets called if a sound 
